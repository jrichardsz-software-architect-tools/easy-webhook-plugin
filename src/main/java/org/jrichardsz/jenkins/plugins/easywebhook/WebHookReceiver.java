...
public class WebHookReceiver implements UnprotectedRootAction {
   ...
   *
   * <p>
   * Adding {@link Action} is primarily useful for plugins to contribute an item to the navigation
   * bar of the top page. See existing {@link Action} implementation for it affects the GUI.
   *
   * <p>
   ...
   *
   * @return Live list where the changes can be made. Can be empty but never null.
   * @since 1.172
   */
  @Override
  ...
  public void doIndex(StaplerRequest req, StaplerResponse resp) throws Exception {
    ...
    LOGGER.log(Level.INFO, "From: " + req.getRemoteHost());
    ...
    try {
      ...
      JenkinsUtil.createBadRequestErrorStaplerResponseAndFinalize(resp,
          finalMessage + " " + exception.getLocalizedMessage());
    } catch (Exception exception) {
      String finalMessage = "Webhook event was received successfully but fail to process it.";
      LOGGER.log(Level.SEVERE, finalMessage, exception);
      JenkinsUtil.createInternalErrorStaplerResponseAndFinalize(resp,
          finalMessage + " " + exception.getLocalizedMessage());
    }
  }
